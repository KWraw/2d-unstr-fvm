/*
  ...
  Last modified: "2015-07-27 12:10:47 kaiwen"
*/
...
void ReadMSHFile(char * filename, GRID * g){
   ...
   while(1){
      ...
      /*--------------------------------------------------------------+
       |   读 nodes, 注意: GMSH 文件中点的编号是从 1 开始的, 预处理网 |
       |   格保留这个设置, 在输出的时候再修改为从 0 开始编号.         |
       +--------------------------------------------------------------*/
   ...
   for(n=1;n<(g->nelem+1);++n){
      ...
      for(i=0;i<NEdge;i++){
         ...
         /* g->en[n][i]=-1000; */
         g->ee[n][i]=DEFAULT_NBR;
         g->en[n][i]=DEFAULT_NBR;
      }
   }
...
ExportMesh(char * nodefile, char * elementfile, GRID * g){
   ...
   for ( n = 0; n < g->nelem; ++n){
      fprintf(fp,"%6d: %6d %6d %6d %6d %6d %6d %6d\n", n,
              g->en[n+1][0]-1,g->en[n+1][1]-1,g->en[n+1][2]-1,
              g->ee[n+1][0]>0 ? g->ee[n+1][0]-1 : g->ee[n+1][0],
              g->ee[n+1][1]>0 ? g->ee[n+1][1]-1 : g->ee[n+1][1],
              g->ee[n+1][2]>0 ? g->ee[n+1][2]-1 : g->ee[n+1][2],
              g->em[n+1]);
   }
   fclose(fp);
